...
import (
	...
	"github.com/golib/revel/harness"
	"github.com/golib/revel/modules/testrunner/app/controllers"
	"io"
	"io/ioutil"
	"net/http"
	"os"
...
For example, to run the booking sample application's tests:
    ...
    revel test github.com/golib/revel/samples/booking dev
...
func testApp(args []string) {
	...
	for _, module := range revel.Modules {
		if module.ImportPath == "github.com/golib/revel/modules/testrunner" {
...
You can add it to a run mode configuration with the following line:
	...
	module.testrunner = github.com/golib/revel/modules/testrunner
...
`)
	...
	Templater := revel.NewTemplateLoader("default", []string{path.Join(module.Path, "app", "views")})
