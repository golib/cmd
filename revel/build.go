...
For example:
    ...
    revel build github.com/golib/revel/samples/chat /tmp/chat
`,
}
...
func buildApp(args []string) {
	...
	mustRenderTemplate(
		runShPath,
		path.Join(revel.RevelPath, "../revelc/scripts", "package_run.sh.template"),
		tmplData)
	...
	mustRenderTemplate(
		...
		path.Join(revel.RevelPath, "../revelc/scripts", "package_run.bat.template"),
		tmplData)
}
